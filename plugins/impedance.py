...
def get_Microstrip_Cap(w: float, h: float, length: float, epsilon_r: float):
    ...
    if w < h:
        C = epsilon_r * length / (60 * v_0 * log(8 * h / w + w / 4 / h))
    else:
        C = (
            (epsilon_r * length)
...
def get_Microstrip_Z0(w: float, h: float, length: float, epsilon_r: float):
...
def get_Plate_Cap(w: float, h: float, length: float, epsilon_r: float):
    C = epsilon_0 * epsilon_r * w * length / h
    return C
...
def get_Coplanar_Cap(w: float, gap: float, length: float, epsilon_r: float):
    ...
    if gap <= 1 / sqrt(2):
        x = pow(1 - s * s, 1 / 4)
        C = (epsilon_r * length) * log(-2 / (x - 1) * (x + 1)) / (377 * pi * v_0)
    else:
        C = (epsilon_r * length) / (120 * v_0 * log(-2 / (sqrt(s) - 1)) * (sqrt(s) + 1))
    return C


def get_Stripline_Cap(w: float, h: float, length: float, epsilon_r: float):
    ws = w / (h / 2)
    factor = epsilon_r * length / (30 * pi * v_0)
...
if __name__ == "__main__":
    ...
    length = 1000 * 1e-3  # Length in m
    epsilon_r = 4.6  # Relative Permittivity

    gap = 100 * 1e-3  # Gap in m (only Coplanar)

    print("Microstrip C:", get_Microstrip_Cap(w, h, length, epsilon_r) * 1e12, "pF")
    print("Microstrip Z0:", get_Microstrip_Z0(w, h, length, epsilon_r), "Ohm")
    print("Stripline C:", get_Stripline_Cap(w, h * 2, length, epsilon_r) * 1e12, "pF")
    print("Plate C:", get_Plate_Cap(w, h, length, epsilon_r) * 1e12, "pF")
    print("Coplanar C:", get_Coplanar_Cap(w, gap, length, epsilon_r) * 1e12, "pF")
