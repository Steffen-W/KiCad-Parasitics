...
def reset() -> None:
    """Reset global ID/node counters. Must be called before generating a new document."""
    global _next_id, _next_node
    _next_id = 0
    _next_node = 1


def _base_item(typ: str, layer: int) -> Item:
    return {
        "type": typ,
        "id": _new_id(),
        "layer": [layer],
...
def make_pad(
    pos: Pos2D, width: float = 0.0002, layer: int = 0, name: str = ""
) -> tuple[Item, int]:
    """Create a PAD at *pos*."""
    x, y = pos
    node = _new_node()
    r = width / 2
    item = _base_item("PAD", layer)
    item.update(
        {
            "position": (x, y),
            "size": (width, width),
...
def make_wire(
    ...
    ns = node_start if node_start is not None else _new_node()
    ne = node_end if node_end is not None else _new_node()
    dx, dy = x2 - x1, y2 - y1
    length = math.hypot(dx, dy) or 1e-30
    wr = width / 2
    nx, ny = -dy / length * wr, dx / length * wr
    item = _base_item("WIRE", layer)
    item.update(
        {
            "start": (x1, y1),
            "end": (x2, y2),
...
def make_via(
    x: float, y: float, drill: float = 0.0003, layers: list[int] | None = None
) -> tuple[Item, NodeMap]:
    """Create a VIA at (*x*, *y*)."""
    if layers is None:
...
class _CoilBuilder:
    ...
    def __init__(
        ...
        self.prev_node = node_start if node_start is not None else _new_node()
...
def generate_circular_coil(
    ...
    cb = _CoilBuilder(r0, 0.0, width, 0)
...
def generate_rectangular_coil(
    ...
    cb = _CoilBuilder(hw0, -hh0, width, 0)
...
def generate_polygon_coil(
    ...
    cb = _CoilBuilder(*_vertex(inner_apothem, 0), width, 0)
    for t in range(turns):
...
def generate_elliptical_coil(
    ...
    cb = _CoilBuilder(inner_rx, 0.0, width, 0)
    for i in range(turns * 4):
        a1 = i * d_angle
...
if __name__ == "__main__":
    ...
    for cb in [
        generate_circular_coil(turns=t, width=w, spacing=s),
        generate_square_coil(turns=t, width=w, spacing=s).move((0.006, 0.0)),
        ...
        generate_polygon_coil(turns=t, width=w, spacing=s, sides=6).move((0.0, 0.006)),
        ...
        pad1, _ = make_pad(cb.start_pos, width=w, name="1", layer=cb.layer)
        pad2, _ = make_pad(cb.end_pos, width=w, name="2", layer=cb.layer)
    ...
    wire, _ns, _ne = make_wire(
        tx, ty, bx, by, width=w, layer=BOTTOM_LAYER, node_start=top.end_node
    )
    via_item, _NodeMap = make_via(tx, ty, layers=[TOP_LAYER, BOTTOM_LAYER])
    pad1, _ = make_pad(top.start_pos, width=w, name="1", layer=TOP_LAYER)
    pad2, _ = make_pad(bot.end_pos, width=w, name="2", layer=BOTTOM_LAYER)
